...
func (g *Generator) genVendor(w io.Writer, vendor *dictionary.Vendor) {
	...
	p(w, `	for _, attr := range p.Attributes[26] {`)
	...
	p(w, `	for _, a := range p.Attributes[26] {`)
	...
	p(w, `	for i := 0; i < len(p.Attributes[26]); {`)
	p(w, `		vendorID, vsa, err := radius.VendorSpecific(p.Attributes[26][i])`)
	p(w, `		if err != nil || vendorID != _`, ident, `_VendorID {`)
	p(w, `			i++`)
	p(w, `			continue`)
	p(w, `		}`)
	...
	p(w, `		}`)
	p(w, `		if len(vsa) > 0 {`)
	p(w, `			copy(p.Attributes[26][i][4:], vsa)`)
	p(w, `			i++`)
	p(w, `		} else {`)
	p(w, `			p.Attributes[26] = append(p.Attributes[26][:i], p.Attributes[26][i+i:]...)`)
	p(w, `		}`)
	p(w, `	}`)
